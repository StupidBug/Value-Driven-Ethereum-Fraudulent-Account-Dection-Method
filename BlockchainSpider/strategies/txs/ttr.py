...
from BlockchainSpider.utils.token_price import TokenPrice
import numpy as np
import logging
        ...
        if node == self.source and node not in self._vis:
            ...
            for e in edges:
                ...
                if token_symbol in ['WETH', 'native']:
                    token_contract = ''
                e['value'] = e.get('value', 0) / (10 ** int(e.get('tokenDecimal', 18)))
                block_number = e.get('blockNumber')
                timestamp = e.get('timeStamp')
                if not e.get('value') == 0:
                    token_price = TokenPrice(token_contract).get_price_at_specific_block(block_number, timestamp)
                    logger.warning("Token {} price is {} at block number {} timestamp {}".format(e.get('symbol'),
                                                                                                 token_price,
                                                                                                 block_number,
                                                                                                 timestamp))
...
class TTRAlpha(TTR):
    ...
    def push(self, node, edges: list, **kwargs):
        ...
        if node == self.source and node not in self._vis:
            ...
            for e in edges:
                ...
                if not e.get('value') == 0:
                    ...
                    logger.warning("Token {} price is {} at block number {} timestamp {}".format(e.get('symbol'),
                                                                                                 ...
                                                                                                 block_number,
                                                                                                 timestamp))
                    value = token_price * e.get('value')
                    edge_weight[str(e)] = value
                    if value > 10000000:
                        print(value)
                else:
                    edge_weight[str(e)] = 0
            ...
            for e in edges:
                # first self push
                if e.get('from') == self.source and edge_weight.get(str(e)) != 0:
                    self.p[self.source] = self.p.get(self.source, 0) + self.get_adaptive_alpha(edge_weight.get(str(e)),self.total_weight*self.beta) * edge_weight.get(str(e))
                    if self.r.get(e.get('to')) is None:
                        self.r[e.get('to')] = list()
                    value = (1 - self.get_adaptive_alpha(edge_weight.get(str(e)), self.total_weight*self.beta)) * self.beta * edge_weight.get(str(e))
                    if value > 0:
                        self.r[e.get('to')].append(dict(
                            value=value,
                            timestamp=e.get('timeStamp'),
                            symbol=e.get('symbol'),
                            ratio=value/(sum_weight * self.beta)
                        ))
                elif e.get('to') == self.source and edge_weight.get(str(e)) != 0:
                    self.p[self.source] = self.p.get(self.source, 0) + self.get_adaptive_alpha(edge_weight.get(str(e)),self.total_weight*(1-self.beta)) * edge_weight.get(str(e))
                    if self.r.get(e.get('from')) is None:
                        self.r[e.get('from')] = list()
                    value = (1 - self.get_adaptive_alpha(edge_weight.get(str(e)), self.total_weight*(1-self.beta))) * (1 - self.beta) * edge_weight.get(str(e))
                    if value > 0:
                        self.r[e.get('from')].append(dict(
                            value=value,
                            timestamp=e.get('timeStamp'),
                            symbol=e.get('symbol'),
                            ratio=value/(sum_weight * (1 - self.beta))
                        ))
            return
        ...
        for chip in r:
            sum_r += chip.get('value', 0)
        self._self_push(node, r, sum_r)
        self._forward_push(node, agg_es, r, sum_r)
        self._backward_push(node, agg_es, r, sum_r)
    ...
    def _forward_push(self, node, aggregated_edges: list, r: list, sum_r):
        ...
        for i in range(0, len(aggregated_edges)):
            ...
            for profit in output_profits:
                inc = self.beta * profit.value * d.get(profit.symbol, 0)
                if inc == 0:
                    continue
    ...
    def _backward_push(self, node, aggregated_edges: list, r: list, sum_r):
        ...
        for i in range(len(aggregated_edges) - 1, -1, -1):
            ...
            for profit in input_profits:
                inc = (1 - self.beta) * profit.value * d.get(profit.symbol, 0)
                if inc == 0:
                    continue
